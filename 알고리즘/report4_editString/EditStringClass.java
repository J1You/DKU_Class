...
public class EditStringClass {
	...
	private int VI, VD, VC, Vmin;  // 각 연산에 대한 비용을 저장 및 최소값 저장을 위한 변수
	...
	public EditStringClass(int n, int m, String x, String y) {
		...
		cost = new int[Lx+1] [Ly+1];  // 비용배열 동적 할당
		edit = new char[Lx+1][Ly+1];  // 연산배열 동적 할당
		
		cost[0][0] = 0;  // 연산이 적용되지 않으므로 첫번째는 0
		edit[0][0] = '-';
		
		for (int i=1; i<=Lx; i++) {  // 첫 번째 열은 삭제 연산만 적용
			cost[i][0] = cost[i-1][0] + 1;
			edit[i][0] = 'D';
		}
		for (int j=1; j<=Ly; j++) {  // 첫 번째 행은 삽입 연산만 적용
		...
		for (int i = 1; i <= Lx; i++) {
			for (int j=1; j<=Ly; j++) {
				...
				else {  // 최소 비용이 발생하는 경우 확인
					VC = cost[i-1][j-1] + 2;  // 교체비용+2
					VD = cost[i-1][j] + 1;  //삭제비용+1
					VI = cost[i][j-1] + 1;  // 삽입비용+1
					
					if (VC <= VD && VC <= VI) {  // 교체비용이 가장 작은 경우
						Vmin = VC;
						edit[i][j] = 'C';
					}
					else if (VD <= VC && VD <= VI) {  // 삭제비용이 가장 작은 경우
						Vmin = VD;
						edit[i][j] = 'D';
					}
					else if (VI <= VC && VI <= VD) {  // 삽입비용이 가장 작은 경우
						Vmin = VI;
						edit[i][j] = 'I';
					}
		...
		System.out.print("   ");
		for (int i=0; i<=Ly; i++) {  
			System.out.print(i+"  ");  // Y의 길이 = 열의 개수
		}
		System.out.println();
